...
D <- OHAS.df %>% filter(MODE %in% c("walk","bike")) %>% group_by(MODE,CTFIP) %>% summarise(TOTDUR = sum(TRPDUR)) %>% arrange(CTFIP,MODE)
D2 <- OHAS.df %>% group_by(CTFIP) %>% summarise(n = n_distinct(paste0(SAMPN,PERNO)))
full_join(D,D2,by="CTFIP") %>% mutate(mean = TOTDUR/n) %>% arrange(MODE,desc(mean)) %>% as.data.frame()
```

## Age Class $\times$ Sex $\times$ County
